...
This file provides guidance to AI coding assistants when working with the vsql_cube extension.
...
In-memory representation (fully allocated for up to 100 dims):
...
On disk, a `cube(n)` column stores `8 + 2*n*8` bytes per row. Serialization uses little-endian
byte order for ndim.

### Key constants

```cpp
constexpr int    kAbsoluteMaxDims    = 100;   // compile-time ceiling
constexpr int    kDefaultMaxDims     = 32;    // bare cube = cube(32)
...
name. `persisted_length(-1)` is required to enable `cube(N)` DDL — a non-negative value causes
the server to treat the type as fixed-length and reject parameterized syntax.
...
(1608 bytes). The framework uses this as the allocation ceiling; actual bytes written is set via
`result->actual_len`. Forgetting this causes NULL returns with no error.
...
Unquoted `cube` in column type position fails with a parser error. VDF constructors return the
base `cube` type without column parameters; inserting VDF results into `` `cube`(N) `` columns
fails with a type-key mismatch. Use string literals for all column INSERTs.
